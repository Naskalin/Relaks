<template>
    <div class="row q-gutter-x-lg items-center q-mb-md">
        ...
        <div class="col flex justify-center">
            <q-input v-model="listStore.listRequest.search" debounce="300" label="Поиск заметок" style="width: 500px"/>
        </div>
        <div class="col-auto">
    ...
    <div v-if="listStore.notes.length" class="q-gutter-y-lg">
        <entry-note-list-item v-for="eText in listStore.notes"
                              :e-text="eText"
                              :key="eText.id"
                              @showEditForm="showEditForm(eText)"></entry-note-list-item>
    </div>
    <div v-else>
        Ничего не найдено
    </div>
...
import {useEntryTextNoteListStore} from "../../../store/entry_notes/entryText.note.list.store";
...
const listStore = useEntryTextNoteListStore();
...
const eTextEditable = ref<EntryText | null>(null);
const showEditForm = (eText: EntryText) => {
    editStore.model = apiMappers.toUpdateEntryTextRequest(eText);
    isShowEdit.value = true;
    eTextEditable.value = eText;
}
const onSubmitEditForm = async () => {
    if (!eTextEditable.value) throw Error('eTextEditable not found');
    const eTextId = eTextEditable.value.id;
